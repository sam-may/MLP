...
void IsoMLTree::Init(TTree *tree) {
  ...
  mlp_branch = tree->GetBranch("mlp");
  if (mlp_branch) mlp_branch->SetAddress(&mlp_);
...
const float &IsoMLTree::mlp() {
  if (not mlp_isLoaded) {
    if (mlp_branch != 0) {
      mlp_branch->GetEntry(index);
    } else {
      printf("branch mlp_branch does not exist!\n");
      exit(1);
    }
    mlp_isLoaded = true;
  }
...
const float &mlp() { return isoml.mlp(); }

...
for d in parseData("/home/jmcauley/datasets/leptons/parsed.json.gz"):
  if d['lepton_fl'] != 1: # To skip either muons or electrons
    continue
  x1 = [1] + d['lepVec']
  x2 = list(calcSummaryVariables(7,8,1, d['lepVec'], d['X']).flatten())
  X1.append(numpy.array(x1, dtype = numpy.float32))
  X2.append(numpy.array(x2, dtype = numpy.float32))
  re.append(d['lepton_re'])
...
weights = {
    ...
    'h21': tf.Variable(tf.random_normal([n_input2, n_hidden_1])), # Weights
    'h22': tf.Variable(tf.random_normal([n_hidden_1, n_hidden_2])),
    'h23': tf.Variable(tf.random_normal([n_hidden_2, n_hidden_3])),
    'h31': tf.Variable(tf.random_normal([n_hidden_3 + n_hidden_3, n_hidden_1])), # Weights
    ...
    'b21': tf.Variable(tf.random_normal([n_hidden_1])), # Bias terms
    'b22': tf.Variable(tf.random_normal([n_hidden_2])),
    'b23': tf.Variable(tf.random_normal([n_hidden_3])),
...
def curve(pred_base, pred, y):
  ...
  y_pred = []
  for i in range(200):
    tpr, fpr = testError(y, pred, i * 1.0 / 100)
    y_pred.append(tpr)
    x_pred.append(fpr)
    tpr, fpr = testError(y, pred_base, - i * 1.0 / 100)
    y_base.append(tpr)
    x_base.append(fpr)
  fig, ax = plt.subplots()
